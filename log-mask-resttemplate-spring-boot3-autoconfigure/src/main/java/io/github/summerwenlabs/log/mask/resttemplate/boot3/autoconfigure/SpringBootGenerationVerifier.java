 ...
 * the auto-configuration can load Spring Framework 6-specific implementation.
 *
...
public final class SpringBootGenerationVerifier implements EnvironmentPostProcessor {
    ...
    private static final String BOOT2_MARKER =
            "META-INF/log-mask-resttemplate-spring-boot2.marker";
    ...
    public void postProcessEnvironment(
        ...
        verify(
                ...
                classLoader.getResource(BOOT2_MARKER) != null);
    }

    static void verify(String springBootVersion, boolean boot2AdapterPresent) {
        if (boot2AdapterPresent) {
            String matchingStarter = springBootVersion != null
                    && springBootVersion.startsWith("2.")
                            ? BOOT2_STARTER : BOOT3_STARTER;
        ...
        if (springBootVersion != null && !springBootVersion.startsWith("3.")) {
            throw new IllegalStateException(
                    "Log Mask RestTemplate Spring Boot 3 starter requires Spring Boot 3.x, "
                            + "but detected Spring Boot " + springBootVersion + ". Replace "
                            + BOOT3_STARTER + " with " + BOOT2_STARTER + ".");
        }
    }
}
